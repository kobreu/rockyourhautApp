...
@class PWAuthorization;
#import "PWCustomerVerification.h"
#import "PWCustomerVerificationFactory.h"
...
/**
 * Defines the methods and properties of the payment provider. Use the factory method getProviderWithName:andCredentials: to instantiate a new payworks provider.
 */
 ...
 * The PWPaymentParamsFactory used for generating the PWPaymentParams associated with this PWPaymentProvider
 */
@property (nonatomic, strong) PWCustomerVerificationFactory* customerVerificationFactory;
...
/**
 * Method creates and registers a transaction that can be used for preauthorizing a transaction. Depending on the PWPaymentParams a PWTransaction with the payment and provider specific processing is created. At the moment only credit card and direct debit transactions are supported. This method also registers the transaction with the processing system. This means the transaction is assigned a unique mobile identifier, which can be used for checking the status later on if the connection is interrupted or an unexpected error occurs. This makes sure that the status of a transaction can always be determined. This method works asynchronously as it needs to contact the processing system to obtain a unique mobile identifier. This does not process the transaction, i.e. no money is transfered. It is solely used to register the transaction:
 * 
 * This framework is built to enable secure and reliable transaction processing, even in challenging mobile environments. The mobile identifier obtained when creating and registering the transaction can always be used to get the status of the current transaction. Make sure to save the identifier before processing a transaction.
 *
 * @param params The parameters in form of a PWPaymentParams object describing the transaction
...
/**
 * Method persists the customer verification (e.g. a signature for a credit card transaction)
 * @param verification the verification to be persisted to external storage
 * @param transaction the transaction associated with the verification data
 * @param success A Block executed when the transaction was processed successfully
 * @param failure A Block executed when the transaction processing failed
 */
-(void)persistCustomerVerification:(PWCustomerVerification *) verification forTransaction: (PWTransaction*) transaction onSuccess:(void(^)(PWTransaction* transaction)) success onFailure:(void(^)(PWTransaction* transaction, NSError* error)) failure;
 ...
 */
/**
 * Factory method for obtaining a new provider. This takes the provider name as well as your profile token followed by your application token. Don't forget to nil terminate the list.
 * - Application id: This identifies your application. This is the same across each installation of your app.
 * - Profile token: This identifies the merchant and how the payment is routed. Depending on your application, you may want to retrieve the profile token from your backend.
 * 
 * @param name The name of the provider, i.e. payworks
 * @param credentials The credentials needed for authenticating. First your profile token, then the application token
 ...
 */
+(PWPaymentProvider*) getProviderWithName:(NSString*) name andCredentials: (NSString*) credentials,...;
@end
